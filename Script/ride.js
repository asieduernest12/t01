...
let gpsError = (error) => { throw alert('failed to record position') }

function recordLocationAndTimeStamp(ride) {
    return (position) => ride.waypoints.push({ utc_timestamp: new Date().toISOString(), position:{coords:{longitude:position.coords.longitude,latitude: position.coords.latitude}} })
}
